...
import { addAlmacen } from '@/services/almacen.services';
import * as XLSX from 'xlsx';
import { toast } from 'react-hot-toast';

const AlmacenesImportModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Plantilla Almacenes");
        XLSX.writeFile(workbook, "plantilla_almacenes.xlsx");
    };
        ...
        reader.onload = async (e) => {
            try {
                ...
                for (const [index, row] of jsonData.entries()) {
                    const nombre = row.nombre || row.nom_almacen || "";
                    const id_sucursal = row.id_sucursal || 1; // Default to 1 if missing, or handle error

                    if (!nombre) {
                        errorCount++;
                        errors.push(`Fila ${index + 2}: Falta nombre del almacén.`);
                        continue;
                    }
                    ...
                    const almacenData = {
                        ...
                        estado_almacen: row.estado !== undefined ? row.estado : 1
                    };

                    const res = await addAlmacen(almacenData);

                    if (res && res.id_almacen) {
                        successCount++;
                    } else {
                        errorCount++;
                ...
                if (successCount > 0) {
                    setResult({
                        success: true,
                        message: `Se importaron ${successCount} almacenes correctamente. ${errorCount > 0 ? `Fallaron ${errorCount}.` : ''}`,
                        details: { errors }
                    });
                    onSuccess();
                } else {
                    setResult({
                        success: false,
                        message: "No se pudo importar ningún almacén.",
                        details: { errors }
                    });
                }
            ...
            <ModalContent>
                <ModalHeader className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-gray-500 font-normal">
                        Sube un archivo Excel para crear almacenes masivamente.
                    </p>
                </ModalHeader>
                <ModalBody>
                    {!result ? (
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800 flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
                                        Descarga la plantilla con las columnas requeridas: nombre, id_sucursal, ubicacion, estado.
                                    </p>
                                </div>
                                <Button
                                    size="sm"
...
export default AlmacenesImportModal;
