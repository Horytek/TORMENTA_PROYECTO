...
export const generateReceiptContent = async (
  venta,
  ignored, // Previously datosVenta, now redundant
  comprobante1,
  observacion,
  nombre,
  empresaData
  ...
  const now = new Date();
  const hora_creacion = venta?.hora_creacion
    ? getHora(venta.hora_creacion, true)
    : getHora(now);
  const hora_impresion = getHora(now, true);

  const descuentoVenta = Number(venta.descuento_raw || venta.descuento_venta || 0);
  const totalT = Number(venta.total_raw || venta.total_t || 0);
  const igv = Number(venta.igv_raw || venta.igv || 0);
  const recibido = Number(venta.recibido_raw || venta.recibido || 0);
  const vuelto = Number(venta.vuelto_raw || venta.vuelto || 0);
  ...
  appendContent(centerText(
    (
      "** " +
      ((venta.tipoComprobante || venta.comprobante_pago)?.toUpperCase() + ": " + (comprobante1?.nuevoNumComprobante || 'N/A')) +
      " **"
    ),
    34
  ));
  ...
  appendContent("==================================");
  if (venta.fechaEmision || venta.fecha)
    appendContent("Fecha de Emisión: " + (venta.fechaEmision || venta.fecha));
  appendContent(""); // Salto de línea extra
  // Sucursal y dirección solo si existen
  if (venta.direccion || venta.direccion_cliente) {
    appendContent(leftAlignText("Dirección: " + (venta.direccion || venta.direccion_cliente), 34));
    appendContent(""); // Salto de línea extra
  }
  if (venta.nombre_sucursal || venta.sucursal)
    appendContent(leftAlignText("Sucursal: " + (venta.nombre_sucursal || venta.sucursal)));

  // Cleaned up redundant checks

  appendContent("==================================");
  // --- Cliente ---
  const clienteNombre = venta.cliente || venta.nombre_cliente;
  if (clienteNombre)
    appendContent(leftAlignText("CLIENTE: " + clienteNombre));

  const clienteDoc = venta.ruc || venta.documento_cliente;
  if (clienteDoc)
    appendContent("RUC/DNI: " + clienteDoc);

  if (venta.direccion) { // Already printed above? Logic was duplicated in original for some reason ("Dirección" vs "direccion_cliente"). We keep "Direction" as global or client specific?
    // Original code had "datosVenta?.direccion" (Branch address?) AND "datosVentaComprobante.direccion_cliente".
    // Let's assume venta.direccion is BRANCH address in one context and CLIENT address in another?
    // In `ventas.controller.js` `getVentas`: cl.direccion as direccion. So it's CLIENT address.
    // But above we used it as branch address? No, original used `datosVenta.direccion` as branch?
    // Let's check `ventas.controller.js` line 316. `cl.direccion` is selected. 
    // `s.ubicacion` is branch address.
    // OK. `venta.ubicacion` is Branch Address. `venta.direccion` is Client Address.
  }
  // Re-correcting block based on Logic:

  if (venta.ubicacion) {
    appendContent(leftAlignText("Dirección Sucursal: " + venta.ubicacion, 34));
    appendContent("");
  }

  appendContent("==================================");
  if (clienteNombre) appendContent(leftAlignText("CLIENTE: " + clienteNombre));
  if (clienteDoc) appendContent("RUC/DNI: " + clienteDoc);
  if (venta.direccion) {
    appendContent(leftAlignText("Dirección: " + venta.direccion, 34));
    appendContent("");
  }
  appendContent("==================================");
  ...
  appendContent(centerText("Forma Pago: " + (venta.metodo_pago || venta.formadepago || 'N/A')));
  ...
  appendContent(centerText("Fec.Regist: " + (venta.fechaEmision || venta.fecha || 'N/A')));
