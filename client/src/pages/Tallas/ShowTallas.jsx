...
import { deleteTalla } from "@/services/talla.services";
import EditTalla from "./EditTalla";
import ConfirmationModal from "@/components/Modals/ConfirmationModal";

const columns = [
    { name: "NOMBRE", uid: "nombre" },
    { name: "ORDEN", uid: "orden" },
    { name: "ACCIONES", uid: "acciones" },
];

export function ShowTallas({ searchTerm, tallas, setTallas, onUpdate, onDelete }) {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            setTallas((prev) => prev.filter((t) => t.id_talla !== id));
        } catch (error) {
            console.error(error);
        }
    };

    const handleUpdate = (updatedTalla) => {
        setTallas((prev) =>
            prev.map((t) =>
                t.id_talla === updatedTalla.id_talla ? { ...t, ...updatedTalla } : t
            )
        );
    };
    ...
    const renderCell = useCallback((item, columnKey) => {
        ...
        switch (columnKey) {
            ...
            case "orden":
                return (
                    <p className="text-sm text-slate-500">{cellValue}</p>
                );
            case "acciones":
                return (
        ...
        <>
            <div className="w-full space-y-4">
                ...
                >
                    ...
                    <TableBody items={items} emptyContent={"No hay tallas registradas."}>
                        {(item) => (
                            <TableRow key={item.id_talla}>
            ...
            {isEditModalOpen && selectedRow && (
                <EditTalla
                    ...
                    modalTitle={"Editar talla"}
                    onClose={handleCloseEditModal}
                    initialData={selectedRow}
                    onTallaEdit={handleUpdate}
                />
            )}
        </>
    );
