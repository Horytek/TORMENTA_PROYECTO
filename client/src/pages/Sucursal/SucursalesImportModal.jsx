...
import { insertSucursal } from '@/services/sucursal.services';
import * as XLSX from 'xlsx';
import { toast } from 'react-hot-toast';

const SucursalesImportModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Plantilla Sucursales");
        XLSX.writeFile(workbook, "plantilla_sucursales.xlsx");
    };
        ...
        reader.onload = async (e) => {
            try {
                ...
                for (const [index, row] of jsonData.entries()) {
                    const nombre = row.nombre || row.nombre_sucursal || "";
                    const id_vendedor = row.id_vendedor || 1; // Default or handle error

                    if (!nombre) {
                        errorCount++;
                        errors.push(`Fila ${index + 2}: Falta nombre de la sucursal.`);
                        continue;
                    }
                    ...
                    const sucursalData = {
                        ...
                        estado_sucursal: row.estado !== undefined ? row.estado : 1
                    };

                    const res = await insertSucursal(sucursalData);
                ...
                if (successCount > 0) {
                    setResult({
                        success: true,
                        message: `Se importaron ${successCount} sucursales correctamente. ${errorCount > 0 ? `Fallaron ${errorCount}.` : ''}`,
                        details: { errors }
                    });
                    onSuccess();
                } else {
                    setResult({
                        success: false,
                        message: "No se pudo importar ninguna sucursal.",
                        details: { errors }
                    });
                }
            ...
            <ModalContent>
                <ModalHeader className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-gray-500 font-normal">
                        Sube un archivo Excel para crear sucursales masivamente.
                    </p>
                </ModalHeader>
                <ModalBody>
                    {!result ? (
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800 flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
                                        Descarga la plantilla con las columnas requeridas: nombre, id_vendedor, ubicacion, estado.
                                    </p>
                                </div>
                                <Button
                                    size="sm"
...
export default SucursalesImportModal;
