    ...
    <>
      <div className="w-full space-y-4">
        ...
        <div className="block md:hidden space-y-4">
          {items.map((item) => (
            <div key={item.id_categoria} className="bg-white dark:bg-zinc-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-zinc-800 space-y-3">
              <div className="flex justify-between items-center gap-2">
                ...
                <Chip
                  size="sm"
                  variant="flat"
                  color={item.estado_categoria === 1 ? "success" : "danger"}
                ...
                >
                  ...
                  <span className="ml-2 text-[10px] items-center">
                    {item.estado_categoria === 1 ? "Activo" : "Inactivo"}
                  </span>
                </Chip>
              </div>

              <div className="pt-3 border-t border-slate-100 dark:border-zinc-800 flex justify-between items-center">
                <span className="text-xs text-slate-400">ID: {item.id_categoria}</span>
                <div className="flex gap-1">
                  <Button isIconOnly size="sm" variant="light" color="primary" onPress={() => hasEditPermission && handleOpenEditModal(item.id_categoria, item.nom_categoria, item.estado_categoria)} isDisabled={!hasEditPermission}>
                    <MdEdit size={16} />
                  </Button>
                  <Button isIconOnly size="sm" variant="light" color="danger" onPress={() => hasDeletePermission && handleOpenConfirmationModal(item.nom_categoria, item.id_categoria)} isDisabled={!hasDeletePermission}>
                    <FaTrash size={14} />
                  </Button>
                </div>
              </div>
          ...
          {items.length === 0 && (
            <div className="text-center p-4 text-slate-500">No hay categorías correspondientes/existentes.</div>
          )}
        </div>
