...
/** Embebido como pestaña dentro de ProductsPage — sin ruta/header propios. */
export default function ContentPage() {
  const { can } = usePermissions();
  const canEdit = can("productos.edit") || can("productos.create");
    ...
    <div className="space-y-6 animate-fade-in">
      <Tabs value={tab} onValueChange={setTab} className="w-full space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-semibold tracking-tight text-foreground">Atributos y variantes</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Define color, talla u otros atributos y qué categorías los usan.
            </p>
          </div>
