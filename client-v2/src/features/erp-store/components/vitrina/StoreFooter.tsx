    ...
    <footer className="mt-auto border-t store-hairline bg-[var(--vitrina-elevated)] text-[var(--vitrina-ink)]">
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10 flex flex-col sm:flex-row gap-8 sm:items-center sm:justify-between">
        ...
        <nav className="flex flex-wrap gap-2 text-sm">
          <Link to={`/s/${slug}`} className="store-nav-btn px-2.5 py-1.5 hover:text-[var(--vitrina-accent)]">
            Catálogo
          </Link>
          <Link to={`/s/${slug}/carrito`} className="store-nav-btn px-2.5 py-1.5 hover:text-[var(--vitrina-accent)]">
            Carrito
          </Link>
          <Link to={`/s/${slug}/opiniones`} className="store-nav-btn px-2.5 py-1.5 hover:text-[var(--vitrina-accent)]">
            Opiniones
          </Link>
          {tienda.telefono && (
