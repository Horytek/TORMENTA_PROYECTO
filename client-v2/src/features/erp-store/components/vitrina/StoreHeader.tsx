...
import { buyerUnreadNotificaciones, getStorefrontToken } from "../../api/erpStore";
...
export function StoreHeader({
  ...
  const openNotificaciones = () => {
    if (!buyerToken) {
      navigate(`/s/${slug}/login`, { state: { from: `/s/${slug}` } });
      return;
    }
    setNotifOpen(true);
  };
    ...
    <>
      <header className={`sticky top-0 z-40 ${headerBg}`}>
        <div className="flex items-center gap-1 sm:gap-2 px-2.5 sm:px-4 lg:px-8 h-14">
          ...
          <Link to={`/s/${slug}`} className="flex items-center gap-2 min-w-0 shrink">
            {tienda.logo_url ? (
          ...
          {showNav && (
            <nav className="store-header-nav hidden lg:flex items-center gap-1 flex-1 min-w-0 mx-4 overflow-x-auto">
          ...
          <div className="flex items-center gap-0 shrink-0">
            ...
            <Link
              to={buyerToken ? `/s/${slug}/cuenta/favoritos` : `/s/${slug}/login`}
            ...
            <Link
              to={buyerToken ? `/s/${slug}/cuenta` : `/s/${slug}/login`}
              className="store-icon-btn size-10 sm:size-11 flex items-center justify-center"
