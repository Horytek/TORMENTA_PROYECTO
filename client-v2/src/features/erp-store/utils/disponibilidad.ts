...
export const DEFAULT_DISP_CONFIG: DisponibilidadConfig = {
  ...
  reserva_al_aprobar: false,
  reserva_minutos: 30,
  permitir_aprobacion_parcial: false,
  congelar_precio_al_aprobar: false,
  permitir_solicitud_invitado: false,
};

/** ERP nunca usa solicitudes de stock, aunque el JSON guardado las traiga. */
export function erpDispConfig(
  partial?: Partial<DisponibilidadConfig> | null
): DisponibilidadConfig {
  return { ...DEFAULT_DISP_CONFIG, ...(partial || {}), solicitudes_activas: false };
}
...
export function requiresSolicitud(opts: {
  ...
  const cfg = erpDispConfig(opts.config);
...
export function resolveDisponibilidad(
  ...
  const cfg = erpDispConfig(config);
  ...
  } else if (estado === "consultar" || metodo === "consultar") {
    const blockCart = metodo === "consultar";
    cta = {
      primary: "whatsapp",
      showCart: !blockCart,
      showWhatsapp: true,
      allowAddToCart: !blockCart,
      requiresSolicitud: false,
      showEnviarSolicitud: false,
    };
