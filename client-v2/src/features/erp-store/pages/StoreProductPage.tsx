...
import { getStore, getStoreProduct, getProductAvailability, validateCartStore, buyerCrearSolicitud } from "../api/erpStore";
...
export default function StoreProductPage() {
  ...
  const onEnviarSolicitud = async () => {
    ...
    if (!token) {
      ...
      navigate(`/s/${slug}/login?next=/s/${slug}/producto/${productId}`);
      return;
    }
    try {
      ...
      navigate(`/s/${slug}/cuenta/solicitudes`);
    } catch (e) {
      toast.error((e as Error).message);
    }
  };
  ...
  if (!tienda || !producto || productQ.isError || !productQ.data?.success) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-[#0a0e14] text-slate-400">
        ...
        <Link to={`/s/${slug}`} className="text-sm font-semibold underline">
          Volver a la tienda
        </Link>
      </div>
    );
          ...
          <div className="flex flex-col gap-4">
            ...
            {disp?.cta.showEnviarSolicitud && (
              <div className="space-y-2">
                ...
                <Link to={`/s/${slug}/cuenta/solicitudes`} className="text-xs underline store-muted">
                  Ver mis solicitudes
                </Link>
              </div>
            )}
