...
export default function StorePaymentResultPage() {
  ...
  try {
    const raw = sessionStorage.getItem(`erp-store-pickup-${slug}`);
        ...
        <div className="mt-6 flex flex-col gap-2">
          {(viewKey === "success" || confirmed) && (
            <Button asChild variant="default">
              <Link to={`/s/${slug}/cuenta/pedidos`}>Ver mis pedidos</Link>
            </Button>
          )}
          <Button asChild variant={viewKey === "success" || confirmed ? "outline" : "default"}>
            <Link to={`/s/${slug}`}>Volver a la tienda</Link>
          </Button>
        </div>
      </div>
    </div>
