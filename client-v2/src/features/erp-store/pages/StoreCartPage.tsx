...
export default function StoreCartPage() {
  ...
  }, [slug, fulfillment, subtotal, id_sucursal, idDestino, idZona, lat, lng, distrito, items.length]);
  ...
  const checkoutMut = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (res.data?.pickup?.direccion) {
        sessionStorage.setItem(`erp-store-pickup-${slug}`, JSON.stringify(res.data.pickup));
      }
      clear();
  ...
  return (
    <StoreShell tienda={tienda} slug={slug}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-12 lg:py-16 flex-1 w-full pb-28 lg:pb-16">
        <Link to={`/s/${slug}`} className="text-sm store-muted hover:text-[var(--vitrina-accent)] min-h-11 inline-flex items-center">
          ← Volver a la tienda
        </Link>
                ...
                <div className="rounded-lg bg-[var(--vitrina-accent-soft)] p-4 text-center space-y-3">
                  ...
                  <div className="flex gap-2 justify-center">
                    <Button asChild size="sm">
                      <Link to={`/s/${slug}/login`} state={{ from: `/s/${slug}/carrito` }}>
                        Iniciar sesión
                      </Link>
                    </Button>
