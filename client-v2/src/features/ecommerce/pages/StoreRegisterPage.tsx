...
import { buyerRegister, getStore } from "../api/ecommerce";
...
export default function StoreRegisterPage() {
  ...
  const regMut = useMutation({
    ...
    onSuccess: (res) => {
      ...
      navigate(`/tienda/${slug}/cuenta`, { replace: true });
    },
    ...
    <StoreShell tienda={tienda || { slug, nombre: slug, color_primario: "#0E7C7B" }} slug={slug}>
      <div className="max-w-md mx-auto px-4 py-16">
        ...
        <p className="text-sm text-center mt-6 store-muted">
          ¿Ya tienes cuenta?{" "}
          <Link to={`/tienda/${slug}/login`} className="text-[var(--vitrina-accent)] font-medium">
            Inicia sesión
          </Link>
        </p>
      </div>
