...
import { buyerLogin, getStore } from "../api/ecommerce";
...
export default function StoreLoginPage() {
  ...
  const from = (location.state as { from?: string })?.from || `/tienda/${slug}/cuenta`;
    ...
    <StoreShell tienda={tienda || { slug, nombre: slug, color_primario: "#0E7C7B" }} slug={slug}>
      <div className="max-w-md mx-auto px-4 py-16">
        ...
        <p className="text-sm text-center mt-6 store-muted">
          ¿No tienes cuenta?{" "}
          <Link to={`/tienda/${slug}/registro`} className="text-[var(--vitrina-accent)] font-medium">
            Regístrate
          </Link>
        </p>
      </div>
