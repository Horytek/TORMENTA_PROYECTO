...
import {
  ...
  type StoreTheme,
  type ThemePreset,
} from "@/features/erp-store/types/theme";
import { getCategoria } from "@/features/erp-store/types/storefront";
import { TiendaWhatsAppSettingsTab } from "../components/TiendaWhatsAppSettingsTab";
import { TiendaWebSettingsCard } from "../components/TiendaWebSettingsCard";
...
export default function TiendaAdminConfigPage() {
  ...
  const { data: catalogoData } = useQuery({
    queryKey: ["tienda-public-catalog", slug],
  ...
  const tienda = cfg
    ? {
        slug,
        nombre: String(cfg.nombre_publico || ""),
        telefono: String(cfg.whatsapp || ""),
        descripcion: String(cfg.mensaje_bienvenida || ""),
        color_primario: String(cfg.color_primario || "#0E7C7B"),
        logo_url: (cfg.logo_url as string) || null,
        theme_json: cfg.theme_json,
      }
    : null;

  const catalogCategorias = useMemo(() => {
    const list = (catalogoData?.productos || []) as { categoria?: string | null; attrs_json?: unknown }[];
  ...
  const saveBrand = useMutation({
    mutationFn: () =>
      adminPatchTiendaConfig({
        ...
        whatsapp: telefono.trim() || null,
        mensaje_bienvenida: descripcion.trim() || null,
    ...
    onSuccess: () => {
      ...
      if (slug) qc.invalidateQueries({ queryKey: ["store", slug] });
    },
  ...
  const uploadLogo = useMutation({
    mutationFn: async (file: File) => {
      ...
      return adminUploadTiendaBrand("logo", b64, file.name);
    },
    onSuccess: (res) => {
  ...
  const uploadBanner = useMutation({
    mutationFn: async (file: File) => {
      const b64 = await fileToBase64(file);
      return adminUploadTiendaBrand("banner", b64, file.name);
    },
    onSuccess: (res) => {
      if (res.success && res.data?.url) {
  ...
  const publicUrl = tienda?.slug ? `${window.location.origin}/s/${tienda.slug}` : "";
    ...
    <div className="space-y-6 max-w-5xl pb-8">
      ...
      <Tabs defaultValue="tienda" className="gap-5">
        ...
        <TabsContent value="whatsapp">
          <TiendaWhatsAppSettingsTab />
        </TabsContent>

        <TabsContent value="cobros">
          <TiendaWebSettingsCard showAdminLink={false} />
        </TabsContent>
      </Tabs>
