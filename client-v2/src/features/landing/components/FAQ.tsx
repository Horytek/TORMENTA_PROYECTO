...
/**
 * FAQ con dos formatos:
 * - Standard: lista vertical con `<details>` (acordeón), 5 preguntas.
 * - Pocket: grilla 3 columnas, 3 tarjetas (estilo del cliente original).
 */
  ...
  return (
    <section
      ...
      className={cn(
        ...
        isPocket ? "bg-amber-500/[0.03]" : "bg-background",
      )}
    >
      <div className="mx-auto max-w-3xl px-6">
        <div className="text-center">
          <span className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">
            {isPocket ? "Dudas Frecuentes · Pocket" : "Preguntas frecuentes"}
          </span>
          <h2 className="mt-3 text-balance text-[clamp(1.75rem,3.5vw,2.5rem)] font-semibold leading-[1.1] tracking-[-0.02em] text-foreground">
            {isPocket ? "Resolvemos tus dudas en segundos." : "Lo que preguntan antes de empezar."}
          </h2>
        </div>

        {isPocket ? <PocketCards /> : <StandardAccordion />}
      </div>
    </section>
  );
}
