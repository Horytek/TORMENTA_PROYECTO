...
export function GuideDetailPanel({ guide, canGeneratePdf }: GuideDetailPanelProps) {
  const [generating, setGenerating] = useState(false);
  const detalles = guide.detalles ?? [];
  ...
  const handlePdf = async () => {
    ...
    try {
      ...
      doc.text("GUIA DE REMISION", pageWidth - 15, 18, { align: "right" });
      doc.setFont("helvetica", "normal").setFontSize(10);
      doc.text(guide.num_guia || "SIN DOC", pageWidth - 15, 24, { align: "right" });

      let y = 40;
      doc.setDrawColor(220).line(15, y - 5, pageWidth - 15, y - 5);
      doc.setFont("helvetica", "bold").setFontSize(9);
      doc.text("DESTINATARIO:", 15, y);
      doc.setFont("helvetica", "normal");
      doc.text(guide.cliente || "-", 55, y);
      y += 5;
      doc.setFont("helvetica", "bold");
      doc.text("MOTIVO:", 15, y);
      doc.setFont("helvetica", "normal");
      doc.text(guide.concepto || "-", 55, y);
      y += 5;
      doc.setFont("helvetica", "bold");
      doc.text("FECHA:", 15, y);
      doc.setFont("helvetica", "normal");
      doc.text(guide.fecha || "-", 55, y);
      y += 5;
      doc.setFont("helvetica", "bold");
      doc.text("SUCURSAL:", 15, y);
      doc.setFont("helvetica", "normal");
      doc.text(guide.nombre_sucursal || "-", 55, y);
      y += 8;

      const rows = detalles.map((d) => [String(d.codigo), d.marca, d.descripcion, String(d.cantidad), d.um]);
      ...
      if (guide.observacion) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const finalY = (doc as any).lastAutoTable?.finalY ?? y + 20;
        doc.setFont("helvetica", "bold").setFontSize(9);
        doc.text("OBSERVACIÓN:", 15, finalY + 8);
        doc.setFont("helvetica", "normal");
        doc.text(doc.splitTextToSize(guide.observacion, pageWidth - 30), 15, finalY + 13);
      }

      doc.save(`${guide.num_guia || "guia-remision"}.pdf`);
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <div className="space-y-3 pt-3">
      ...
      {guide.observacion && (
        <p className="rounded-lg bg-muted/40 px-3 py-2 text-xs text-muted-foreground">{guide.observacion}</p>
      )}
        ...
        {detalles.map((d, i) => (
          <div key={`${d.codigo}-${i}`} className="flex items-center justify-between gap-3 px-3 py-2">
            ...
            <div className="shrink-0 text-right text-sm">
              ...
              <span className="ml-1 text-xs text-muted-foreground">{d.um}</span>
            </div>
          </div>
        ))}
        {detalles.length === 0 && (
